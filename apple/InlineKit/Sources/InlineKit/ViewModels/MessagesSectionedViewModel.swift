...
public class MessagesSectionedViewModel {
  ...
  public init(peer: Peer, reversed: Bool = false) {
    self.progressiveViewModel = MessagesProgressiveViewModel(peer: peer, reversed: reversed)
  ...
  private func convertToSectionedChangeSet(from update: MessagesProgressiveViewModel.MessagesChangeSet) -> SectionedMessagesChangeSet {
    switch update {
    case .reload(let animated):
      rebuildSections()
    ...
    case .updated(let updatedMessages, _, let animated):
  ...
  public func message(at indexPath: IndexPath) -> FullMessage? {
    guard indexPath.section >= 0,
          ...
          indexPath.item >= 0,
          indexPath.item < sections[indexPath.section].messages.count else {
      log.warning("Invalid index path: section=\(indexPath.section), item=\(indexPath.item), sectionsCount=\(sections.count)")
      return nil
    }
  ...
  public func section(for date: Date) -> MessageSection? {
    return sections.first { Self.calendar.isDate($0.date, inSameDayAs: date) }
  }
}