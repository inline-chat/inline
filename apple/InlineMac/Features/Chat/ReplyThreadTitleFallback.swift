...
enum ReplyThreadTitleFallback {
  ...
  static func parentTitlesByChatId(for items: [HomeChatItem], db: Database) throws -> [Int64: String] {
    try parentTitlesByChatId(for: items.compactMap(\.chat), db: db)
  }

  static func parentTitlesByChatId(for chats: [Chat], db: Database) throws -> [Int64: String] {
