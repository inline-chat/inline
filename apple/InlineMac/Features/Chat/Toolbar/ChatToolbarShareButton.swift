...
struct ChatToolbarShareButton: View {
  ...
  @State private var isMakePublicConfirmationPresented = false
  ...
  var body: some View {
    ...
    .confirmationDialog(
      ...
      isPresented: $isMakePublicConfirmationPresented,
  ...
  private func toggleVisibility() {
    ...
    if model.state.isPublic {
      ...
      isMakePublicConfirmationPresented = true
    }
  }
  ...
  private func updateVisibility(isPublic: Bool, participantIds: [Int64]) {
    ...
    Task {
      do {
        _ = try await dependencies.realtimeV2.send(.updateChatVisibility(
          chatID: chatId,
          isPublic: isPublic,
          participantIDs: participantIds
        ))
        do {
          try await dependencies.realtimeV2.send(.getChatParticipants(chatID: chatId))
        } catch {
...
private struct ChatToolbarShareState: Equatable {
  ...
  init(chat: Chat, currentUserId: Int64?, db: Database) throws {
    ...
    let membership = try chat.spaceId.flatMap { spaceId in
      try Member
        .filter(Column("spaceId") == spaceId)
        .filter(Column("userId") == currentUserId)
        .fetchOne(db)
    }
    let canManage = ChatVisibilityPolicy.canChange(
      chat: chat,
      currentUserId: currentUserId,
