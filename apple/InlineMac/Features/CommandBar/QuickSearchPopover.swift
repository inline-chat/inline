...
final class QuickSearchViewModel {
  ...
  @ObservationIgnored private let catalog = InlineSearchChatCatalog()
  ...
  @ObservationIgnored private var spaceSearchTask: Task<Void, Never>?
  @ObservationIgnored private var localProjectionToken: UInt64 = 0
  @ObservationIgnored private var catalogUpdateTask: Task<Void, Never>?
  ...
  private var isSpaceSearching = false
  ...
  func performSearch() {
    ...
    searchSpaces(query: searchQuery, generation: generation)
  ...
  private func bindCatalog() {
    dependencies.database.homeChatListItemSnapshotsPublisher()
      .receive(on: RunLoop.main)
      .sink { [weak self] snapshots in
        guard let self else { return }
        catalogUpdateTask?.cancel()
        catalogUpdateTask = Task { [weak self, catalog] in
          guard Task.isCancelled == false else { return }
          await catalog.replace(snapshots)
          guard Task.isCancelled == false, let self else { return }
          self.runLocalProjection(query: self.activeSearchQuery)
        }
      }
  ...
  private func searchSpaces(query: String, generation: UInt64) {
    guard supportsSpaceSelection, let preparedQuery = InlineSearchMatcher.prepare(query) else { return }
    isSpaceSearching = true
    spaceSearchTask = Task { [weak self, database = dependencies.database] in
      do {
        let spaces = try await database.reader.read { db in
          try Space.fetchAll(db)
        }
        let rankedSpaces = await Task.detached(priority: .userInitiated) {
          Self.rankSpaces(spaces, query: preparedQuery)
        }.value
        guard Task.isCancelled == false, let self, self.searchGeneration == generation else { return }
        self.spaceResults = rankedSpaces
        self.isSpaceSearching = false
        self.rebuildLocalResults()
      } catch {
  ...
  private func runLocalProjection(query: String) {
    ...
    localProjectionTask = Task { [weak self, catalog, usageStore] in
  ...
  private nonisolated static func rankSpaces(
    _ spaces: [Space],
