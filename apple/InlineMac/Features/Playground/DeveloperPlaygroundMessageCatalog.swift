    ...
    case .richContent:
      [
        ...
        .init(
          ...
          "Codex-style reasoning, grouped exploration, command, raw appendix, and footer hierarchy"
        ),
        .init(
          .richRTLBlocks,
          "Per-block RTL",
...
private enum DeveloperMessageCatalogFactory {
  ...
  private static func richAgentAnswer() -> RichCatalogFixture {
    ...
    let progressSummary = builder.segment("Worked for 2m 05s")
    let reasoningSummary = builder.segment("Mapping the rich-output renderer")
    let reasoningBody = builder.segment(
      "I found that retained provider events were being projected as empty nested disclosures instead of a readable work narrative."
    )
    let explorationSummary = builder.segment("Explored")
    let first = builder.segment("Read stream_ui.rs")
    let second = builder.segment("Searched for reasoning content and activity grouping")
    let commandSummary = builder.segment("Ran cargo test -p inline-cli stream_ui")
    let commandOutput = builder.segment("61 passed; 0 failed")
    let providerSummary = builder.segment("Provider data · 2 items")
    let providerBody = builder.segment("Raw diagnostic payloads stay collapsed in verbose mode.")
    let footer = builder.segment("Generated by OpenClaw · sources checked")
    let exploration = disclosureBlock(
    ...
    let command = disclosureBlock(
      summary: commandSummary,
      children: [codeBlock(commandOutput, language: "text")],
      activity: .command
    )
    let reasoning = disclosureBlock(
      summary: reasoningSummary,
      initiallyOpen: true,
      children: [paragraphBlock(reasoningBody), exploration, command],
      activity: .reasoning
    )
    let providerData = disclosureBlock(
      summary: providerSummary,
      children: [paragraphBlock(providerBody)],
      activity: .tool
    )
    let progress = disclosureBlock(
  ...
  private static func disclosureBlock(
    ...
    isRTL: Bool? = nil,
    activity: BlockDisclosure.ActivityKind = .unspecified
  ) -> InlineProtocol.Block {
    var disclosure = BlockDisclosure()
    disclosure.summary = summary
    disclosure.kind = kind
    disclosure.activityKind = activity
