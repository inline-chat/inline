...
struct AddParticipantsSheet: View {
  ...
  init(
    ...
    _viewModel = StateObject(
      wrappedValue: AddParticipantsViewModel(
        chatId: chatId,
        spaceId: spaceId,
    ...
    ScrollView {
      LazyVStack(spacing: 0) {
        ForEach(viewModel.filteredMembers, id: \.userInfo.user.id) { member in
          MemberRow(
            member: member,
            isSelected: viewModel.selectedUserIds.contains(member.userInfo.user.id),
            onTap: {
              viewModel.toggleSelection(userId: member.userInfo.user.id)
            }
          )
        }
      }
