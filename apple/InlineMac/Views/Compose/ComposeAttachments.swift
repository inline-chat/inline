...
final class ComposeAttachments: NSView {
  ...
  private func addPendingDocument(
    fileName: String,
    fileSize: Int?,
    reservesThumbnailSpace: Bool,
    id: String
  ) {
    documentModels[id] = .pending(
      PendingDocumentPresentation(
        fileName: fileName,
        fileSize: fileSize,
        reservesThumbnailSpace: reservesThumbnailSpace
      )
    )
...
private final class PendingDocumentAttachmentView: NSView {
  ...
  override func layout() {
    ...
    let textX = mediaFrame.maxX + DocumentPresentationPlan.mediaSpacing
