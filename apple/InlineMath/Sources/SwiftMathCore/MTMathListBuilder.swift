...
public struct MTMathListBuilder {
    ...
    // Inline: bounds include unbraced fractions/scripts and nested environments.
    private var parseDepth = 0
    private var parsedTableCells = 0
    ...
    public mutating func buildInternal(_ oneCharOnly: Bool, stopChar stop: Character?) -> MTMathList? {
        guard parseDepth < 32 else {
            setError(.internalError, message: "Formula nesting limit exceeded")
            return nil
        }
        parseDepth += 1
        defer { parseDepth -= 1 }
        ...
        while self.hasCharacters {
            ...
            if char == "^" {
                ...
                if atom == nil {
                    // Inline must not silently drop unsupported source glyphs.
                    // Whitespace is ordinary TeX spacing; everything else
                    // falls back to the original formula at the caller.
                    if char.isWhitespace { continue }
                    setError(.invalidCommand, message: "Unsupported formula character")
                    return nil
                }
            }
    ...
    mutating func readColor() -> String? {
        ...
        // Ignore spaces, never silently discard unsupported source glyphs.
        self.skipSpaces()
        ...
        guard MTColor(fromString: mutable) != nil else {
            setError(.invalidCommand, message: "Unsupported formula color")
            return nil
        }
        return mutable;
    }
    ...
    private mutating func admitTableCell() -> Bool {
        guard parsedTableCells < 256 else {
            setError(.internalError, message: "Formula table limit exceeded")
            return false
        }
        parsedTableCells += 1
        return true
    ...
    mutating func buildTable(env: String?, firstList: MTMathList?, isRow: Bool) -> MTMathAtom? {
        ...
        if firstList != nil {
            guard admitTableCell() else { return nil }
        ...
        while !currentEnv!.ended && self.hasCharacters {
            ...
            if currentEnv!.numRows > currentRow {
                // A row separator inside an unfinished argument can advance
                // the shared environment more than once before a cell returns.
                // Such a cell is malformed; do not index an unallocated row.
                guard currentEnv!.numRows == currentRow + 1 else {
                    setError(.invalidEnv, message: "Row separator inside formula argument")
                    return nil
                }
        ...
        // A ragged matrix is padded by the factory. Bound that rectangular
        // allocation too, not only the number of explicitly parsed cells.
        let columnCount = rows.map(\.count).max() ?? 0
        guard rows.count * columnCount <= 256 else {
            setError(.internalError, message: "Formula table limit exceeded")
            return nil
        }
    ...
    mutating func readDelimiter() -> String? {
        ...
        while self.hasCharacters {
            let char = self.getNextCharacter()
            guard char >= "\u{21}" && char <= "\u{7E}" else {
                setError(.invalidDelimiter, message: "Unsupported formula delimiter")
                return nil
            }
            if char == "\\" {
