...
// Inline supports modern Core Text on every deployment target.
private func isIos6Supported() -> Bool { true }
...
public class MTFractionDisplay : MTDisplay {
    ...
    override public func draw(_ context:CGContext) {
        ...
        if let color = self.textColor { context.setStrokeColor(color.cgColor) }
...
class MTRadicalDisplay : MTDisplay {
    ...
    override public func draw(_ context: CGContext) {
        ...
        context.saveGState();
        if let color = self.textColor { context.setStrokeColor(color.cgColor) }
        if let color = self.textColor { context.setFillColor(color.cgColor) }
