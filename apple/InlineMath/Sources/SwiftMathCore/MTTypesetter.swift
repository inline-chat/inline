    ...
    switch type {
        ...
        case .radical:
            if row {
                ...
                return 0 // A radical on the right has ordinary spacing.
            }
        default:
            return nil
    }
...
enum MTTypesettingFailure: Error {
    case resourceLimit, invalidMetrics
}

/// Shared only by the recursive typesetters for one formula, never across
/// renders. Bounds iterative glyph assembly before bitmap allocation.
final class MTTypesettingBudget {
    private var remaining = 50_000

    func consume(_ count: Int) throws {
        guard count >= 0, count <= remaining else { throw MTTypesettingFailure.resourceLimit }
        remaining -= count
    }
}

class MTTypesetter {
    let budget: MTTypesettingBudget
    var font:MTFont!
    ...
    static func createLineForMathList(_ mathList:MTMathList?, font:MTFont?, style:MTLineStyle, budget: MTTypesettingBudget = MTTypesettingBudget()) throws -> MTMathListDisplay? {
        let finalizedList = mathList?.finalized
        // default is not cramped
        return try self.createLineForMathList(finalizedList, font:font, style:style, cramped:false, budget: budget)
    }
    
    // Internal
    static func createLineForMathList(_ mathList:MTMathList?, font:MTFont?, style:MTLineStyle, cramped:Bool, budget: MTTypesettingBudget) throws -> MTMathListDisplay? {
        return try self.createLineForMathList(mathList, font:font, style:style, cramped:cramped, spaced:false, budget: budget)
    }
    
    // Internal
    static func createLineForMathList(_ mathList:MTMathList?, font:MTFont?, style:MTLineStyle, cramped:Bool, spaced:Bool, budget: MTTypesettingBudget) throws -> MTMathListDisplay? {
        assert(font != nil)
        guard mathList != nil, font != nil else { throw MTTypesettingFailure.invalidMetrics }
        try budget.consume(mathList!.atoms.count + 1)
        let preprocessedAtoms = self.preprocessMathList(mathList)
        let typesetter = MTTypesetter(withFont:font, style:style, cramped:cramped, spaced:spaced, budget:budget)
        ...
        guard [line.width, line.ascent, line.descent].allSatisfy({ $0.isFinite && $0 >= 0 })
        else { throw MTTypesettingFailure.invalidMetrics }
        guard line.width <= 4096, line.ascent + line.descent <= 4096
        else { throw MTTypesettingFailure.resourceLimit }
        return line
    }
    
    static var placeholderColor: MTColor { MTColor.blue }
    
    init(withFont font:MTFont?, style:MTLineStyle, cramped:Bool, spaced:Bool, budget: MTTypesettingBudget) {
        self.budget = budget
        self.font = font
    ...
    func addInterElementSpace(_ prevNode:MTMathAtom?, currentType type:MTMathAtomType) throws {
        var interElementSpace = CGFloat(0)
        if prevNode != nil {
            interElementSpace = try getInterElementSpace(prevNode!.type, right:type)
        } else if self.spaced {
            // For the first atom of a spaced list, treat it as if it is preceded by an open.
            interElementSpace = try getInterElementSpace(.open, right:type)
        }
    ...
    func createDisplayAtoms(_ preprocessed:[MTMathAtom]) throws {
        ...
        for atom in preprocessed {
            switch atom.type {
                ...
                case .color:
                    ...
                    let display = try MTTypesetter.createLineForMathList(colorAtom.innerList, font: font, style: style, budget: budget)
                    display!.localTextColor = MTColor(fromString: colorAtom.colorString)
                ...
                case .textcolor:
                    ...
                    if prevNode != nil {
                        // Color wrappers are ordinary atoms in the spacing
                        // table. Their contents may be empty or a fraction,
                        // matrix, etc.; there need not be a first glyph line.
                        let interElementSpace = try self.getInterElementSpace(prevNode!.type, right: .ordinary)
                        if currentLine.length > 0 {
                ...
                case .colorBox:
                    ...
                    let display = try MTTypesetter.createLineForMathList(colorboxAtom.innerList, font:font, style:style, budget: budget)
                    
                    display!.localBackgroundColor = MTColor(fromString: colorboxAtom.colorString)
                    display!.position = currentPosition
                    currentPosition.x += display!.width;
                    displayAtoms.append(display!)
                ...
                case .radical:
                    ...
                    try self.addInterElementSpace(prevNode, currentType:.ordinary)
                    let displayRad = try self.makeRadical(rad.radicand, range:rad.indexRange)
                    if rad.degree != nil {
                        // add the degree to the radical
                        let degree = try MTTypesetter.createLineForMathList(rad.degree, font:font, style:.scriptOfScript, budget: budget)
                    ...
                    if atom.subScript != nil || atom.superScript != nil {
                        try self.makeScripts(atom, display:displayRad, index:UInt(rad.indexRange.location), delta:0)
                    }
                ...
                case .fraction:
                    ...
                    try self.addInterElementSpace(prevNode, currentType:atom.type)
                    let display = try self.makeFraction(frac)
                    displayAtoms.append(display!)
                    currentPosition.x += display!.width;
                    // add super scripts || subscripts
                    if atom.subScript != nil || atom.superScript != nil {
                        try self.makeScripts(atom, display:display, index:UInt(frac!.indexRange.location), delta:0)
                    }
                ...
                case .inner:
                    ...
                    if inner!.leftBoundary != nil || inner!.rightBoundary != nil {
                        ...
                        display = try MTTypesetter.createLineForMathList(inner!.innerList, font:font, style:style, cramped:cramped, budget: budget)
                    }
                    display!.position = currentPosition
                    currentPosition.x += display!.width
                    displayAtoms.append(display!)
                    // add super scripts || subscripts
                    if atom.subScript != nil || atom.superScript != nil {
                        try self.makeScripts(atom, display:display, index:UInt(atom.indexRange.location), delta:0)
                    }
                ...
                case .underline:
                    ...
                    let display = try self.makeUnderline(under)
                    displayAtoms.append(display!)
                    currentPosition.x += display!.width;
                    // add super scripts || subscripts
                    if atom.subScript != nil || atom.superScript != nil {
                ...
                case .overline:
                    ...
                    let display = try self.makeOverline(over)
                    displayAtoms.append(display!)
                    currentPosition.x += display!.width;
                    // add super scripts || subscripts
                    if atom.subScript != nil || atom.superScript != nil {
                ...
                case .accent:
                    ...
                    let display = try self.makeAccent(accent)
                    displayAtoms.append(display!)
                    currentPosition.x += display!.width;
                ...
                case .table:
                    ...
                    try self.addInterElementSpace(prevNode, currentType:.inner)
                    atom.type = .inner;
                ...
                case .ordinary, .binaryOperator, .relation, .open, .close, .placeholder, .punctuation:
                    ...
                    if prevNode != nil {
                        let interElementSpace = try self.getInterElementSpace(prevNode!.type, right:atom.type)
                        if currentLine.length > 0 {
                            if interElementSpace > 0 {
                                // add a kerning of that space to the previous character
                                currentLine.addAttribute(kCTKernAttributeName as NSAttributedString.Key,
                    ...
                    if atom.subScript != nil || atom.superScript != nil {
                        ...
                        guard atom.indexRange.length > 0,
                              let scriptIndex = UInt(exactly: NSMaxRange(atom.indexRange) - 1)
                        else { throw MTTypesettingFailure.invalidMetrics }
                        try self.makeScripts(atom, display:line, index:scriptIndex, delta:delta)
                    }
            } // switch
            lastType = atom.type
            prevNode = atom
        ...
        if spaced && lastType != nil {
            ...
            let interElementSpace = try self.getInterElementSpace(lastType, right:.close)
    ...
    func getInterElementSpace(_ left: MTMathAtomType, right:MTMathAtomType) throws -> CGFloat {
        guard let leftIndex = getInterElementSpaceArrayIndexForType(left, row: true),
        ...
        guard spaceType != .invalid else { throw MTTypesettingFailure.invalidMetrics }
    ...
    func makeScripts(_ atom: MTMathAtom?, display:MTDisplay?, index:UInt, delta:CGFloat) throws {
        ...
        if atom!.superScript == nil {
            ...
            let _subscript = try MTTypesetter.createLineForMathList(atom!.subScript, font:font, style:self.scriptStyle(), cramped:self.subscriptCramped(), budget: budget)
        ...
        let superScript = try MTTypesetter.createLineForMathList(atom!.superScript, font:font, style:self.scriptStyle(), cramped:self.superScriptCramped(), budget: budget)
        ...
        let ssubscript = try MTTypesetter.createLineForMathList(atom!.subScript, font:font, style:self.scriptStyle(), cramped:self.subscriptCramped(), budget: budget)
    ...
    func makeFraction(_ frac:MTFraction?) throws -> MTDisplay? {
        // lay out the parts of the fraction
        let fractionStyle = self.fractionStyle;
        let numeratorDisplay = try MTTypesetter.createLineForMathList(frac!.numerator, font:font, style:fractionStyle(), cramped:false, budget: budget)
        let denominatorDisplay = try MTTypesetter.createLineForMathList(frac!.denominator, font:font, style:fractionStyle(), cramped:true, budget: budget)
        ...
        if frac!.leftDelimiter.isEmpty && frac!.rightDelimiter.isEmpty {
            ...
            return try self.addDelimitersToFractionDisplay(display, forFraction:frac)
        }
    }
    
    func addDelimitersToFractionDisplay(_ display:MTFractionDisplay?, forFraction frac:MTFraction?) throws -> MTDisplay? {
        ...
        if !frac!.leftDelimiter.isEmpty {
            let leftGlyph = try self.findGlyphForBoundary(frac!.leftDelimiter, withHeight:glyphHeight())
        ...
        if !frac!.rightDelimiter.isEmpty {
            let rightGlyph = try self.findGlyphForBoundary(frac!.rightDelimiter, withHeight:glyphHeight())
    ...
    func getRadicalGlyphWithHeight(_ radicalHeight:CGFloat) throws -> MTDisplayDS? {
        ...
        if glyphAscent + glyphDescent < radicalHeight {
            ...
            glyphDisplay = try self.constructGlyph(radicalGlyph, withHeight:radicalHeight)
        }
    ...
    func makeRadical(_ radicand:MTMathList?, range:NSRange) throws -> MTRadicalDisplay? {
        let innerDisplay = try MTTypesetter.createLineForMathList(radicand, font:font, style:style, cramped:true, budget: budget)!
        ...
        let glyph = try self.getRadicalGlyphWithHeight(radicalHeight)!
    ...
    func constructGlyph(_ glyph:CGGlyph, withHeight glyphHeight:CGFloat) throws -> MTGlyphConstructionDisplay? {
        ...
        try self.constructGlyphWithParts(parts, glyphHeight:glyphHeight, glyphs:&glyphs, offsets:&offsets, height:&height)
        guard let firstGlyph = glyphs.first else { throw MTTypesettingFailure.invalidMetrics }
        var first = firstGlyph.uint16Value
    ...
    func constructGlyphWithParts(_ parts:[GlyphPart], glyphHeight:CGFloat, glyphs:inout [NSNumber], offsets:inout [NSNumber], height:inout CGFloat) throws {
        guard glyphHeight.isFinite, glyphHeight >= 0, glyphHeight <= 4096,
              !parts.isEmpty, parts.count <= 64 else { throw MTTypesettingFailure.resourceLimit }
        // Bound both the iterative search and total repeated parts across every
        // nested radical/delimiter in this formula.
        for numExtenders in 0..<512 {
        ...
        throw MTTypesettingFailure.resourceLimit
    }
    ...
    func makeLargeOp(_ op:MTLargeOperator!) throws -> MTDisplay?  {
    ...
    func addLimitsToDisplay(_ display:MTDisplay?, forOperator op:MTLargeOperator, delta:CGFloat) throws -> MTDisplay? {
        ...
        if op.limits && style == .display {
            ...
            if op.superScript != nil {
                superScript = try MTTypesetter.createLineForMathList(op.superScript, font:font, style:self.scriptStyle(), cramped:self.superScriptCramped(), budget: budget)
            }
            if op.subScript != nil {
                subScript = try MTTypesetter.createLineForMathList(op.subScript, font:font, style:self.scriptStyle(), cramped:self.subscriptCramped(), budget: budget)
            }
            ...
            currentPosition.x += display!.width;
            try self.makeScripts(op, display:display, index:UInt(op.indexRange.location), delta:delta)
            return display;
        }
    }
    ...
    func makeLeftRight(_ inner: MTInner?) throws -> MTDisplay? {
        assert(inner!.leftBoundary != nil || inner!.rightBoundary != nil, "Inner should have a boundary to call this function");
        
        let innerListDisplay = try MTTypesetter.createLineForMathList(inner!.innerList, font:font, style:style, cramped:cramped, spaced:true, budget: budget)
        ...
        if inner!.leftBoundary != nil && !inner!.leftBoundary!.nucleus.isEmpty {
            let leftGlyph = try self.findGlyphForBoundary(inner!.leftBoundary!.nucleus, withHeight:glyphHeight)
            leftGlyph!.position = position
            position.x += leftGlyph!.width
            innerElements.append(leftGlyph!)
        }
        ...
        if inner!.rightBoundary != nil && !inner!.rightBoundary!.nucleus.isEmpty {
            let rightGlyph = try self.findGlyphForBoundary(inner!.rightBoundary!.nucleus, withHeight:glyphHeight)
            rightGlyph!.position = position;
            position.x += rightGlyph!.width;
            innerElements.append(rightGlyph!)
        }
    ...
    func findGlyphForBoundary(_ delimiter:String, withHeight glyphHeight:CGFloat) throws -> MTDisplay? {
        var glyphAscent=CGFloat(0), glyphDescent=CGFloat(0), glyphWidth=CGFloat(0)
        ...
        if (glyphAscent + glyphDescent < glyphHeight) {
            ...
            glyphDisplay = try self.constructGlyph(leftGlyph, withHeight:glyphHeight)
        }
    ...
    func makeUnderline(_ under:MTUnderLine?) throws -> MTDisplay? {
        let innerListDisplay = try MTTypesetter.createLineForMathList(under!.innerList, font:font, style:style, cramped:cramped, budget: budget)
    ...
    func makeOverline(_ over:MTOverLine?) throws -> MTDisplay? {
        let innerListDisplay = try MTTypesetter.createLineForMathList(over!.innerList, font:font, style:style, cramped:true, budget: budget)
    ...
    func makeAccent(_ accent:MTAccent?) throws -> MTDisplay? {
        var accentee = try MTTypesetter.createLineForMathList(accent!.innerList, font:font, style:style, cramped:true, budget: budget)
        if accent!.nucleus.isEmpty {
            // no accent!
            return accentee
        }
        ...
        if self.isSingleCharAccentee(accent) && (accent!.subScript != nil || accent!.superScript != nil) {
            ...
            accentee = try MTTypesetter.createLineForMathList(accent!.innerList, font:font, style:style, cramped:cramped, budget: budget)
        }
    ...
    func makeTable(_ table:MTMathTable?) throws -> MTDisplay? {
    ...
    func typesetCells(_ table:MTMathTable?, columnWidths: inout [CGFloat]) throws -> [[MTDisplay]] {
        var displays = [[MTDisplay]]()
        for row in table!.cells {
            var colDisplays = [MTDisplay]()
            for i in 0..<row.count {
                let disp = try MTTypesetter.createLineForMathList(row[i], font:font, style:style, budget: budget)
