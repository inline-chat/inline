...
enum ThumbnailFormatRegistry {
  ...
  private static let deniedImageExtensions: Set<String> = ["ico", "icns"]
  ...
  static func descriptor(for url: URL, contentType: UTType?, policy: ThumbnailPolicy) -> ThumbnailFormatDescriptor? {
    ...
    guard !deniedImageExtensions.contains(fileExtension),
    ...
    return raster.isAvailable(policy: policy) ? raster : nil
  }
