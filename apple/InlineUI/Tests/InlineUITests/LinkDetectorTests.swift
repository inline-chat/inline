...
struct LinkDetectorTests {
  ...
  @Test("Detects bare sub-domain links (bot.example.test)")
  func detectsSubdomainBareDomain() async throws {
    let text = "The service is hosted at bot.example.test and is always online."
    let matches = detector.detectLinks(in: text)

    #expect(matches.count == 1, "Should detect exactly one link in the sentence")
    #expect(matches.first?.url.absoluteString == "https://bot.example.test")
  }
