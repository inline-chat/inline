...
struct ProcessEntitiesTests {
  ...
  func testExtractBold() {
    ...
    #if os(macOS)
    let boldFont = NSFontManager.shared.convert(testConfiguration.font, toHaveTrait: .boldFontMask) ?? testConfiguration.font
    #else
  ...
  func testOffsetPreservationWithComplexFormatting() {
    ...
    #if os(macOS)
    let boldFont = NSFontManager.shared.convert(testConfiguration.font, toHaveTrait: .boldFontMask) ?? testConfiguration.font ?? testConfiguration.font
    #else
    let boldFont = UIFont.boldSystemFont(ofSize: testConfiguration.font.pointSize)
    #endif
    attributedString.addAttribute(.font, value: boldFont, range: boldRange)
