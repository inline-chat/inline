...
`sendMessage` parses Markdown by default; set `parse_markdown: false` for literal text. Ordinary messages return plain `text` plus Telegram-aligned UTF-16 `entities`, so mentions are safe to detect without parsing text. Structural messages also return `rich_message.blocks`, whose text-bearing blocks contain recursive rich text. Every message has `peer_id: { user_id } | { chat_id }`; expanded `chat` data is optional and omitted from repeated history and nested replies.
