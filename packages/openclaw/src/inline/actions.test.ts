...
describe("inline/actions", () => {
  ...
  it("dispatches expanded rpc action set via Inline RPC", async () => {
    ...
    const downloadInlineMediaFromUrl = vi.fn(async () => ({
      path: "/tmp/inline-downloads/photo.jpg",
    ...
    await inlineMessageActions.handleAction?.({
      ...
      action: "sendAttachment",
      cfg,
      params: { to: "user:99", filePath: "/workspace/openclaw/half-vertical.png", caption: "local file" },
    } as any)
    ...
    await inlineMessageActions.handleAction?.({
      ...
      action: "send",
      cfg,
      params: { to: "user:99", message: "media alias", media: "/workspace/openclaw/half-vertical.png" },
    } as any)
    ...
    await inlineMessageActions.handleAction?.({
      ...
      action: "upload-file",
      cfg,
      params: { to: "user:99", path: "/workspace/openclaw/report.pdf", message: "file upload" },
    } as any)
    ...
    await inlineMessageActions.handleAction?.({
      ...
      params: {
        ...
        caption: "multi",
        filePaths: [
          "/workspace/openclaw/half-vertical.png",
          "/workspace/openclaw/half-vertical-2.png",
        ],
      },
    } as any)
    ...
    expect(uploadInlineMediaFromUrl).toHaveBeenCalledWith(
      expect.objectContaining({
        mediaUrl: "/workspace/openclaw/half-vertical.png",
      }),
    )
    expect(uploadInlineMediaFromUrl).toHaveBeenCalledWith(
      expect.objectContaining({
        mediaUrl: "/workspace/openclaw/report.pdf",
      }),
    )
    expect(uploadInlineMediaFromUrl).toHaveBeenCalledWith(
      expect.objectContaining({
        mediaUrl: "/workspace/openclaw/half-vertical-2.png",
      }),
    )
  ...
  it("downloads an explicit media URL without opening the Inline client", async () => {
    ...
    const close = vi.fn(async () => {})
    const downloadInlineMediaFromUrl = vi.fn(async () => ({
      path: "/tmp/inline-downloads/report.pdf",
    ...
    expect(result).toMatchObject({
      details: {
        ...
        contentType: "application/pdf",
        media: {
          mediaUrl: "/tmp/inline-downloads/report.pdf",
          mediaUrls: ["/tmp/inline-downloads/report.pdf"],
  ...
  it("downloads current-message media from current Inline chat context", async () => {
    ...
    const close = vi.fn(async () => {})
    const downloadInlineMediaFromUrl = vi.fn(async () => ({
      path: "/tmp/inline-downloads/voice.ogg",
  ...
  it("resolves thread-reply through a saved parent message route", async () => {
    ...
    setInlineRuntime({
      state: {
        resolveStateDir: () => "/tmp/inline-actions-test",
        openKeyedStore,
      },
      logging: {
