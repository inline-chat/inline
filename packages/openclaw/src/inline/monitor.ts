...
function normalizeInlineRequiredId(value: unknown): string | null {
...
export async function monitorInlineProvider(params: {
  ...
  const handleInboundNow = async (input: InlineParsedInboundEvent): Promise<void> => {
    ...
    const senderId = normalizeInlineRequiredId(msg.fromId)
    if (!senderId) {
      const messageId = msg.id != null ? String(msg.id) : "unknown"
      const error = `inline: drop inbound chat=${String(chatId)} message=${messageId} (missing sender id)`
      log?.warn(error)
      statusSink?.({ lastError: error })
      return
    }
