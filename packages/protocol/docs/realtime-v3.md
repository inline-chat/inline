...
For a received sequence range, clients apply the contiguous next sequence, ignore stale/duplicate sequences, and buffer gaps while one authoritative `getUpdates` repair runs. Live application is postponed during repair. Overflow clears the unsafe buffer, records the need for authoritative catch-up, and never silently continues. Session reset, restart, or temporary-key replacement must confirm convergence before the sync owner reports `live`; clients may keep rendering the last durable state with an explicit syncing/degraded classification instead of blocking the whole account UI.
...
`TOO_LONG` currently means the incremental difference exceeds the server's bounded response budget; the server does not currently prune sequenced update rows. Apple replaces a cold chat bucket and continues a warm backlog in bounded background tranches; Rust replaces any affected bucket through its existing authoritative user/space/chat snapshot owner. TypeScript calls the host's optional authoritative bucket-repair callback and advances only after that callback durably replaces the complete bucket and confirms its applied cursor; without that owner it preserves the committed cursor and reports the bucket as degraded through `getSyncStatus()`. No client fast-forwards from server sequence metadata alone. Before update-row retention is introduced, this contract must gain an authenticated retention floor (or equivalent authoritative reseed marker); deleting an unannounced prefix would make a small missing range indistinguishable from corruption.

Sequenced user/space/chat bucket records are durable and recoverable through `getUpdates`. Server updates emitted by the explicitly transient presence/compose owner, direct `GridEvent`, and `BotEvent` are ephemeral/lossy; Grid snapshots repair current media state, while bot interactions have no history contract.
