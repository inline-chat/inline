...
describe("inline/bot-avatar-tool", () => {
  ...
  it("uploads a zip avatar package for the authenticated bot", async () => {
    ...
    const result = await tool?.execute("tool-1", {
      zipPath: "/workspace/downloads/zhu-bajie.zip",
    })

    expect(loadWebMedia).toHaveBeenCalledWith("/workspace/downloads/zhu-bajie.zip", {
      maxBytes: 40_000_000,
      optimizeImages: false,
      localRoots: ["/workspace/downloads"],
    })
  ...
  it("rejects clear requests that also provide avatar package metadata", async () => {
    ...
    await expect(
      tool?.execute("tool-ambiguous-clear", {
        action: "clear",
        source: "/workspace/downloads/zhu-bajie.zip",
      }),
