...
describe("packed artifact", () => {
  it("does not ship unresolved runtime package imports beyond OpenClaw SDK peers", async () => {
    ...
    for (const sdkFile of ["sdk/inline-sdk-client.js", "realtime/protocol-client.js", "utils/async-channel.js"]) {
      const index = bundledMap.sources.findIndex(source => source.endsWith(`/packages/sdk/dist/${sdkFile}`))
      expect(index, `bundled SDK ${sdkFile}`).toBeGreaterThanOrEqual(0)
      expect(bundledMap.sourcesContent[index]).toBe(await readFile(path.resolve(packageDir, "../../packages/sdk/dist", sdkFile), "utf8"))
    }
