...
describe("getChats", () => {
  ...
  test("returns empty collections for a user with no conversations", async () => {
    const user = await testUtils.createUser("empty@example.com")
    const result = await getChats({}, makeHandlerContext(user.id))
    expect(result).toEqual({ chats: [], dialogs: [], spaces: [], users: [], messages: [], folders: [] })
  })
