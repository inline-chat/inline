...
describe("messages.updateDialogOpen", () => {
  ...
  test("opens, unarchives, and unhides an existing dialog", async () => {
    ...
    expect(result.dialog.openedDate).toBeTruthy()
    ...
    expect(dialog?.openedDate).toBeInstanceOf(Date)
  ...
  test("preserves openedDate when an already-open dialog is reopened", async () => {
    ...
    await db
      .update(dialogs)
      .set({ archived: true, chatListHidden: true, open: true, openedDate })
      .where(and(eq(dialogs.chatId, chat.id), eq(dialogs.userId, userA.id)))
  ...
  test("closes open dialogs and clears openedDate", async () => {
    ...
    await db
      .update(dialogs)
      .set({ open: true, openedDate: new Date("2026-01-02T03:04:05.000Z") })
      .where(and(eq(dialogs.chatId, chat.id), eq(dialogs.userId, userA.id)))
