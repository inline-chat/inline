...
export const admin = new Elysia({ name: "admin", prefix: "/admin" })
  ...
  .get(
    ...
    async ({ cookie, request, server, set }) => {
      ...
      const currentDayStart = getStartOfUtcDay(metricsNow)
      const [appMetrics, errorStats, waitlistCountRow, recentActivity, dailyActivity] = await Promise.all([
  ...
  .get(
    ...
    async ({ query, cookie, request, server, set }) => {
      ...
      const activeUsers = await getActiveMetricUsers(period)
      return { ok: true, period, limit: ADMIN_ACTIVE_USERS_LIMIT, users: activeUsers }
    },
    {
      query: t.Object({
        period: t.Optional(t.Union([t.Literal("today"), t.Literal("week")])),
      }),
      cookie: adminCookieSchema,
    },
  )
...
const getDailyActivity = async (currentDayStart: Date) => {
  const firstDayStart = new Date(currentDayStart.getTime() - 13 * 24 * 60 * 60 * 1000)
  const nextDayStart = new Date(currentDayStart.getTime() + 24 * 60 * 60 * 1000)
  const messageDay = sql<string>`to_char(date_trunc('day', ${messages.date}), 'YYYY-MM-DD')`
  const userDay = sql<string>`to_char(date_trunc('day', ${users.date}), 'YYYY-MM-DD')`
  const baseUserWhere = and(
    or(isNull(users.deleted), eq(users.deleted, false)),
    or(isNull(users.bot), eq(users.bot, false)),
  )

  const [activeUserRows, newUserRows] = await Promise.all([
    db
      .select({
        day: messageDay,
      ...
      .from(messages)
      .innerJoin(users, eq(messages.fromId, users.id))
      .where(and(gte(messages.date, firstDayStart), lt(messages.date, nextDayStart), baseUserWhere))
      .groupBy(sql`date_trunc('day', ${messages.date})`),
    db
      .select({
        day: userDay,
        count: sql<number>`count(*)::int`,
      ...
      .from(users)
      .where(and(gte(users.date, firstDayStart), lt(users.date, nextDayStart), baseUserWhere))
      .groupBy(sql`date_trunc('day', ${users.date})`),
  ])

  const activeUsersByDay = new Map(activeUserRows.map((row) => [row.day, row.count]))
  const newUsersByDay = new Map(newUserRows.map((row) => [row.day, row.count]))

  return Array.from({ length: 14 }, (_, index) => {
    const day = new Date(firstDayStart.getTime() + index * 24 * 60 * 60 * 1000)
...
type ActiveUserPeriod = "today" | "week"

const getActiveMetricUsers = async (period: ActiveUserPeriod) => {
  const startOfDay = getStartOfUtcDay(new Date())
  const nextDay = new Date(startOfDay.getTime() + 24 * 60 * 60 * 1000)
  const windowStart =
    period === "today" ? startOfDay : new Date(startOfDay.getTime() - 6 * 24 * 60 * 60 * 1000)
  const baseUserWhere = and(
    or(isNull(users.deleted), eq(users.deleted, false)),
    or(isNull(users.bot), eq(users.bot, false)),
  )
  ...
  const rows = await db
    ...
    .from(messages)
    .innerJoin(users, eq(messages.fromId, users.id))
    .where(and(gte(messages.date, windowStart), lt(messages.date, nextDay), baseUserWhere))
    .groupBy(
      users.id,
      users.email,
      users.firstName,
