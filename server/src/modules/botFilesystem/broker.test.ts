...
describe("private filesystem broker", () => {
  test("only the exact bot can answer; answers are consumed once", async () => {
    ...
    const pending = broker.create(7)!
    expect(broker.answer(pending.id, 8, reply)).toBe(false)
    expect(broker.answer(pending.id, 7, reply)).toBe(true)
    expect(broker.answer(pending.id, 7, reply)).toBe(false)
    expect(await pending.response).toEqual(reply)
  })
  test("per-bot capacity is bounded and shutdown resolves all requests", async () => {
    const broker = new BotFilesystemBroker()
    const pending = Array.from({ length: 8 }, () => broker.create(7)!)
    expect(broker.create(7)).toBeUndefined()
    broker.shutdown()
  ...
  test("offline hosts time out", async () => {
    ...
    expect((await broker.create(7)!.response).result.oneofKind).toBe("problem")
  })
})
