...
test("scrubs SDK request enrichment and transaction spans without mutating the request", () => {
  const event: Event = {
    ...
    request: {
      ...
      headers: { authorization: "Bearer synthetic-auth" }, query_string: "synthetic-query",
    },
    ...
    spans: [{ span_id: "1234567890123456", trace_id: "12345678901234567890123456789012", start_timestamp: 1,
      description: "GET /v1/synthetic-span/logout?otp=synthetic-otp", data: { "http.url": "https://test.invalid/?token=synthetic-url" } }],
