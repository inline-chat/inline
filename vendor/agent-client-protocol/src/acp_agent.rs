...
const ACP_STDOUT_FRAME_LIMIT: usize = 1024 * 1024;
const ACP_STDOUT_READ_CHUNK: usize = 8 * 1024;

fn bounded_stdout_lines<R>(reader: R) -> impl futures::Stream<Item = std::io::Result<String>> + Send
where
    R: futures::AsyncRead + Unpin + Send,
{
    ...
    futures::stream::try_unfold(
        ...
        |(mut reader, mut pending, mut eof)| async move {
            loop {
                if let Some(newline) = pending.iter().position(|byte| *byte == b'\n') {
                    if newline > ACP_STDOUT_FRAME_LIMIT {
                        return Err(std::io::Error::new(
                            std::io::ErrorKind::InvalidData,
                            "ACP stdout frame exceeds the 1 MiB limit",
                        ));
                    }
                    ...
                    let line = String::from_utf8(line).map_err(|_| {
                        std::io::Error::new(
                            std::io::ErrorKind::InvalidData,
                            "ACP stdout frame is not valid UTF-8",
                        )
                    })?;
                    return Ok(Some((line, (reader, pending, eof))));
                }
                if eof {
                    if pending.is_empty() {
                        return Ok(None);
                    }
                    if pending.len() > ACP_STDOUT_FRAME_LIMIT {
                        return Err(std::io::Error::new(
                            std::io::ErrorKind::InvalidData,
                            "ACP stdout frame exceeds the 1 MiB limit",
                        ));
                    }
                    let line = String::from_utf8(std::mem::take(&mut pending)).map_err(|_| {
                        std::io::Error::new(
                            std::io::ErrorKind::InvalidData,
                            "ACP stdout frame is not valid UTF-8",
                        )
                ...
                let mut chunk = [0_u8; ACP_STDOUT_READ_CHUNK];
                ...
                if !pending.contains(&b'\n') && pending.len() > ACP_STDOUT_FRAME_LIMIT {
                    return Err(std::io::Error::new(
                        std::io::ErrorKind::InvalidData,
                        "ACP stdout frame exceeds the 1 MiB limit",
                    ));
...
impl<Counterpart: AcpAgentCounterpartRole> crate::ConnectTo<Counterpart> for AcpAgent {
    async fn connect_to(
        ...
        > = if let Some(callback) = self.debug_callback.clone() {
            Box::pin(bounded_stdout_lines(child_stdout).inspect(move |result| {
                if let Ok(line) = result {
                    callback(line, LineDirection::Stdout);
                }
            }))
        } else {
            Box::pin(bounded_stdout_lines(child_stdout))
        };
...
mod tests {
    ...
    #[tokio::test]
    async fn stdout_lines_are_rejected_before_exceeding_the_frame_limit() {
        use futures::StreamExt;

        let input = futures::io::Cursor::new(vec![b'x'; ACP_STDOUT_FRAME_LIMIT + 1]);
        let mut lines = Box::pin(bounded_stdout_lines(input));
        let error = lines
            .next()
            .await
            .expect("oversized frame result")
            .expect_err("oversized frame must fail");
        assert_eq!(error.kind(), std::io::ErrorKind::InvalidData);
    }

    #[tokio::test]
    async fn stdout_line_decoder_preserves_multiple_bounded_frames() {
        use futures::StreamExt;

        let input = futures::io::Cursor::new(b"one\r\ntwo\nthree".to_vec());
        let lines = bounded_stdout_lines(input)
