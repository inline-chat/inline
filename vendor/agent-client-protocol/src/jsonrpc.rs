...
/// # }
/// ```
const CHANNEL_FRAME_CAPACITY: usize = 64;

/// The bounded sending half of an in-process JSON-RPC transport.
#[derive(Clone, Debug)]
pub struct ChannelSender {
    inner: mpsc::Sender<TransportFrame>,
}

impl ChannelSender {
    /// Try to enqueue a frame without waiting for capacity.
    #[allow(clippy::result_large_err)]
    pub fn unbounded_send(
        &self,
        frame: TransportFrame,
    ) -> Result<(), mpsc::TrySendError<TransportFrame>> {
        self.inner.clone().try_send(frame)
    }

    pub(crate) async fn send(&self, frame: TransportFrame) -> Result<(), mpsc::SendError> {
        self.inner.clone().send(frame).await
    }

    /// Close this sending half so the receiver observes end-of-stream.
    pub fn close_channel(&self) {
        self.inner.clone().close_channel();
    }
}

/// The bounded receiving half of an in-process JSON-RPC transport.
#[derive(Debug)]
pub struct ChannelReceiver {
    inner: mpsc::Receiver<TransportFrame>,
}
...
/// A bounded bidirectional JSON-RPC transport endpoint.
#[derive(Debug)]
pub struct Channel {
...
impl Channel {
    ...
    pub fn duplex() -> (Self, Self) {
        let (a_tx, b_rx) = mpsc::channel(CHANNEL_FRAME_CAPACITY);
        let (b_tx, a_rx) = mpsc::channel(CHANNEL_FRAME_CAPACITY);

        (
            Self {
                rx: ChannelReceiver { inner: a_rx },
                tx: ChannelSender { inner: a_tx },
            },
            Self {
                rx: ChannelReceiver { inner: b_rx },
                tx: ChannelSender { inner: b_tx },
            },
        )
    }
