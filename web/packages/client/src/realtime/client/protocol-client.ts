...
export class ProtocolClient {
    ...
    ;(async () => {
      for await (const event of this.transport.events) {
        switch (event.type) {
          ...
          case "disconnected":
            await this.disconnected(event.reason)
            break
  ...
  private async disconnected(reason?: string) {
    this.state = "connecting"
    this.pingPong.stop()
    this.cancelAllRpcContinuations(
      new ProtocolClientError("not-connected"),
    )
